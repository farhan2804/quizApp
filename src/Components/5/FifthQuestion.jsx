...
const FirstQuestion = () => {
  ...
  const changeScore = () => {
    if (selectedOption !== null && questionsData.length > 0) {
      if (selectedOption === questionsData[4].ans) {
        setCount(count + 1);
      }
    }
  };
    ...
    <>
      <div className="mainContainer">
        ...
        <div className="answerContainer">
          ...
          >
            {questionsData.length > 0 && questionsData[4].optionA}
          </button>
          <button
            className={buttonClass2}
            onClick={() => captureValue2("optionB")}
          >
            {questionsData.length > 0 && questionsData[4].optionB}
          </button>
          <button
            className={buttonClass3}
            onClick={() => captureValue3("optionC")}
          >
            {questionsData.length > 0 && questionsData[4].optionC}
          </button>
          <button
            className={buttonClass4}
            onClick={() => captureValue4("optionD")}
          >
            {questionsData.length > 0 && questionsData[4].optionD}
          </button>
